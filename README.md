...
- 📱 移动端优先界面，首页重做为更接近手机记账 App 的浅色账本布局，适配 iPhone 安全区
...
- `src/pages/HomePage.vue` - 首页，包含 App Header、总览卡片与按日期分组的最近记录
...
- 顶部改为更像原生账本 App 的 Header：中间显示当前账本，下面显示当前月份，月份左右切换按钮紧贴月份区域。
- 首页主卡片改为单张“总支出”总览卡，集中展示本月记录数、共同支出、我本月已付、另一半本月已付。
- 最近记录区改为按日期分组的流水布局：日期行显示当天日期和当天合计，每条记录展示分类图标、消费说明、时间/备注和金额。
- 空状态改为与流水区同风格的轻量展示，不再使用网页式虚线框。
