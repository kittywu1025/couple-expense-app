...
- 📅 收支日历：按天同时展示支出和收入，支出热力图只按支出金额计算
...
- `src/pages/CalendarPage.vue` - 收支日历页，按月显示每日支出热力图、收入金额与当天流水
...
- 收支日历页现在同时显示支出和收入：格子内使用 `-1.2k / +5k` 这类紧凑金额，支出使用红/橙色，收入使用绿色。
- 日历热力图只按当日支出金额计算深浅，收入不会影响支出热力颜色；点击某一天后，当天详情会同时列出支出和收入流水。
- 为兼容尚未补齐 `record_type` 的旧 Supabase 表，云端读取时会对默认收入分类做兜底识别，避免收入被误按支出显示。
...
- 添加记录页的金额和货币现已合并到同一区域，`JPY / CNY` 不再单独占一整块；只有跨货币时才显示汇率输入。
- 分摊方式区域已去掉大块比例说明卡和 `0 / 10 / 20 ...` 按钮网格；自定义比例改成单个输入框，只输入“我的承担比例”，另一半比例自动计算。
- 添加记录页的普通保存已拆成两个动作：
  - `保存并继续`：保存成功后留在添加页，清空金额和说明，保留日期、类型、货币、分类、付款人和分摊方式
  - `保存并退出`：保存成功后返回首页
- 金额输入框现在默认为空，不再预填 `0`；空值或 `<= 0` 时会 toast：`请输入有效金额`
- 添加记录页新增轻量 `快速输入`：
  - 示例：`5月3日 药店买药 1078，超市 2000`
  - 支持 `5月3日 / 5/3 / 2026/5/3 / 今天 / 昨天`
  - 支持逗号、顿号、换行拆分多笔
  - 如果只有第一笔带日期，后续未写日期的项目会沿用同一天
  - 默认按支出处理，默认货币跟随设置页；未写货币时按默认货币
  - 当前第一版只适合快速录入支出；收入会提示 `快速输入暂不支持收入`
- 快速输入会先显示解析预览，不会直接保存；若某条缺日期、金额、说明，或跨货币缺少汇率，会标记“需要补充”并阻止保存。
...
- 快速输入第一版会识别 `JPY / 日元 / 円` 与 `CNY / 人民币 / 元`；如果解析出的货币和默认货币不同，会提示需要补充汇率，改用普通表单更合适。
- 当前是 MVP 实现：
...
- 快速输入的分类会按关键词自动匹配：
  ...
  - `电车 / 公交 / 交通 / IC → 交通`
  - `房租 / 家賃 → 房租`
  - `水费 / 电费 / 煤气 / ガス / 网费 / 网络 / 书 / 教材 / 打印 / 学费 / 衣服 / 美容 / 化妆 / 游戏 / 电影 / 展 / 社交`
  - 其余默认归到 `杂费`
- 添加页默认类型是 `支出`。
- 收入模式下：
  - 分类切到收入分类
...
- 设置页新增了 `固定消费` 入口，进入独立页面管理每月自动生成的固定账单。
...
- 固定消费字段包括：
  ...
  - 分摊方式
  - 是否启用
- 当用户打开 App、进入首页、进入设置页或进入固定消费页时，会检查本月是否存在应生成但尚未生成的固定消费。
- 自动生成规则：
  - `enabled = true`
  - 当前月份还没有生成过
  - 当前日期已达到设定的每月日期
  - 没超过结束日期
- 每月日期支持 `1–31`；如果目标月份没有该日期，会自动落在当月最后一天。
- 防重复策略：
  - 新增 `fixed_expense_runs`
  - 使用 `unique(fixed_expense_id, generated_month)` 保证同一条固定消费每月最多生成一次
- 生成后的账单会复用现有普通支出保存逻辑，生成的记录会附带备注：`由固定消费自动生成（fixed_expense_id）`。
- 生成出的普通记录仍然可以像其他账单一样编辑或删除。
- 如果固定消费表还没在 Supabase 建好，页面会直接 toast：
  - `固定消费数据表还没有创建，请先在 Supabase 执行 SQL。`
...
- 已验证的主要流程：
  ...
  - 当前账号前端真实新增一笔 `JPY 108 / 前端测试 JPY`
  - 当前账号前端真实新增一笔 `CNY 10 / 汇率 21 / 前端测试 CNY`
  - 添加页普通表单实测：
    - 金额输入默认为空，不是 `0`
    - 空金额点击 `保存并继续` 会 toast：`请输入有效金额`
    - 普通 `JPY` 记录可分别通过 `保存并继续` 和 `保存并退出` 成功保存
  - 添加页快速输入实测：
    - 输入 `5月3日 药店买药 1078，超市 2000` 可解析成两笔 `2026/05/03` 的 `JPY` 支出
    - 分类会分别匹配到 `医疗/药品` 和 `超市/食材`
    - 批量保存后首页 / 云端都可见
    - 输入 `5月3日 红包 50元` 会提示 `快速输入暂不支持收入`，不会误保存成支出
  - 固定消费页面 smoke test：
    - 设置页可以进入固定消费页
    - 固定消费表单可正常渲染和交互
    - 当前 Supabase 尚未创建 `fixed_expenses / fixed_expense_runs` 表时，会返回真实错误：
      - `code = PGRST205`
      - `message = Could not find the table 'public.fixed_expenses' in the schema cache`
    - 页面会改为中文 toast：`固定消费数据表还没有创建，请先在 Supabase 执行 SQL。`
- 页面刷新后首页与记录页仍可读到刚保存的云端记录
  - 从记录页进入账单详情后删除 `前端测试 CNY`，首页 / 记录页 / Supabase 三端同步消失
  - 线上生产地址 `https://couple-expense-app.pages.dev/?v=a4567b7` 已完成一轮真实页面流程验证：
    - 邮箱 + 密码登录
    - 刷新后 session 保持
    - 添加 `JPY` 记录
    - 添加 `CNY` 记录并填写汇率
...
- 当前已知限制：
  ...
  - 当前固定消费功能在 Supabase 执行建表 SQL 前无法完成云端新增、自动生成和 runs 去重闭环。
...
- 账号 B：
  - 已成功注册独立测试账号。
  - 当前项目开启邮箱确认，B 账号注册后不会直接拿到 session。
  - 当前再次实测结果：
    - `signUp`: 成功，但 `hasSession = false`
    - `signInWithPassword`: 失败，`code = email_not_confirmed`
  - 因此在未完成邮箱确认前，无法完成“B 登录后输入邀请码加入账本”以及后续 A/B 双向消费同步的最终验证。
- 已验证的配对相关接口结果：
  - 错误邀请码：返回 `邀请码不存在`
