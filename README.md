...
- 设置页底部增加了临时版本标识：`UI版本：2026-05-03-hero-removed`，用于确认线上是否已加载到最新界面。
- 调整了 Service Worker 缓存版本与静态资源更新策略，减少部署后手机端继续命中旧首页和旧导航的问题。
- 前端增加了新版本检测提示。检测到新 Service Worker 后，会显示“发现新版本 / 立即刷新”提示，用户可手动刷新切换到新界面。
...
- 项目启用了 Service Worker 用于离线缓存，因此部署新版本后，手机端可能会在极短时间内继续显示旧页面。
- 现在的更新策略包括：
  - `index.html` 与根路径使用 `no-cache`
  - `service-worker.js` 使用独立版本号注册
  - `assets` 下带 hash 的静态资源允许长缓存
  - 前端检测到新版本后显示“立即刷新”提示
- 如果你需要快速确认线上是否已刷新到本次 UI，可以进入设置页底部查看临时版本文本：`UI版本：2026-05-03-hero-removed`
- 如果仍然异常，可以手动刷新页面；若历史缓存状态异常，再清理浏览器网站数据。
